...
def parse_pdf(
    ...
    if not form_recognizer:
        ...
        form_recognizer_client = DocumentAnalysisClient(
            endpoint=os.environ["FORM_RECOGNIZER_ENDPOINT"], credential=credential
        )
...
def embed_docs_faiss(
    docs: List[Document], chunks_limit: int = 100, verbose: bool = False
) -> VectorStore:
...
def get_sales_search_results(
    ...
    for index in indexes:
        ...
        if vector_search:
            ...
            search_payload[
                "select"
            ] = "id, ServiceName, ServiceType, ServiceCategory, ServiceFamily, Price, Description, InternalComments, chunks, ActionArea, FocusArea, metadata_storage_name, vectorized"
...
def get_answer(
    ...
    answer = chain(
        {"input_documents": docs, "question": query, "language": language},
...
class SalesSearchResults(BaseTool):
    """Tool for Azure Search results"""
...
class DocSearchTool(BaseTool):
    ...
    name = "@docs"
    description = "useful when the questions includes the term: @docs.\n"
    ...
    def _run(
        ...
        try:
            tools = [
                DocSearchResults(
                    indexes=self.indexes,
                    vector_only_indexes=self.vector_only_indexes,
                    k=self.k,
                    reranker_th=self.reranker_th,
                    similarity_k=self.similarity_k,
                    sas_token=self.sas_token,
...
class SalesSearchTool(BaseTool):
    """Tool for Azure GPT Smart Search Engine"""
    ...
    async def _arun(self, query: str) -> str:
        """Use the tool asynchronously."""
        raise NotImplementedError("SalesSearchTool does not support async")
