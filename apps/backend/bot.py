...
class MyBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        input_text_metadata[
            "local_timestamp"
        ] = turn_context.activity.local_timestamp.strftime(
            "%I:%M:%S %p, %A, %B %d of %Y"
        )
        ...
        exsp_search = SalesSearchTool(
            ...
            return_direct=True,
            name="@sales",
            description="useful when the questions includes the term: @sales.\n",
        )
        www_search = BingSearchTool(
            llm=llm, k=5, callback_manager=cb_manager, return_direct=True
        )
        # sql_search = SQLDbTool(
        #     llm=llm, k=10, callback_manager=cb_manager, return_direct=True
        # )
        ...
        memory = ConversationBufferWindowMemory(
            memory_key="chat_history", return_messages=True, k=30, chat_memory=cosmos
        )
        ...
        answer = await loop.run_in_executor(
            ThreadPoolExecutor(), run_agent, input_text, agent_chain
        )
